		...
		foreach($menu_structure[$parent] as $menu)
		{
			...
			if ($menu->menu_type == 'page')
			{
				foreach($pages as $page)
				{
					...
					{
						$menu->menu_name = $page->post_title;
						break;
					}
				}
			}
...
?>
		<li class="ui-widget-content ui-corner-tr<?= ($parent == 0) ? ((has_sub_menu($menu_structure, $menu->menu_id)) ? '' : ' enable-drag') : ''; ?>">
			<div class="head-container">
				<h5 class="ui-widget-header"><?= $menu->menu_name; ?>&nbsp;(<?= $menu->menu_type; ?>)</h5>
			</div>
			<input class="sys_menu_unid" type="hidden" name="main_menu[unid][]" value="<?= $menu->menu_id; ?>" style="width:30px;" />
			<input class="sys_menu_parent" type="hidden" name="main_menu[parent][]" value="<?= $menu->menu_parent; ?>" style="width:30px;" />
			Position: <input type="text" name="main_menu[sort][]" value="<?= $menu->menu_order; ?>" style="width:30px;" />
			...
			<div class="droppedbox">
				<ul class="mm_gallery ui-helper-reset">
					<?php create_menu_dragdrop_item($pages, $categories, $selected_pages_full, $selected_categories_full, $menu_structure, $menu->menu_id); ?>
					<div style="clear:both;"></div>
				</ul>
			</div>
			<div style="clear:both;"></div>
			<a href="#" title="Move out this" class="ui-icon ui-icon-refresh" style="display:<?= ($menu->menu_parent > 0) ? 'block' : 'none'; ?>">Move Out</a>
		</li>
<?php
		}
	}
...
<link type="text/css" media="screen" rel="stylesheet" href="<?= MM_DISPLAY_URL; ?>/styles/jquery/jquery-ui.css" />
<style>
...
<script type="text/javascript" src="<?= MM_DISPLAY_URL; ?>/js/jquery-ui.js"></script>
<script type="text/javascript" src="<?= MM_DISPLAY_URL; ?>/js/menu-manager.js"></script>
						...
						<div class="inside">
							<form name="menu" action="" method="post">
									...
									<table id="newmeta">
										<tr>
											<td class="submit" style="float:none;">
												...
												<div class="item">
													<input id="page_<?= $page->ID; ?>" type="checkbox" class="checkbox" name="page_main_menu[]" value="<?= $page->ID; ?>"<?php if (in_array($page->ID, $selected_pages)) { echo ' checked="checked"'; } ?>/>
													&nbsp;<label for="page_<?= $page->ID; ?>"><?php echo $page->post_title; ?></label>
												</div>
												<?php endforeach; ?>
												<?php endif; ?>
											</td>
									...
									<table id="newmeta">
										<tr>
											<td class="submit" style="float:none;">
												...
												<div class="item">
													<input id="cat_<?= $pc->cat_ID; ?>" type="checkbox" class="checkbox" name="cat_main_menu[]" value="<?= $pc->cat_ID; ?>"<?php if (in_array($pc->cat_ID, $selected_categories)) { echo ' checked="checked"'; } ?>/>
													&nbsp;<label for="cat_<?= $pc->cat_ID; ?>"><?php echo $pc->cat_name; ?></label>
												</div>
												<?php endforeach; ?>
												<?php endif; ?>
											</td>
										...
										<tr>
											<td class="submit" style="float:none;">
												<div id="sys_cust_link_container">
													...
													<div class="item">
														<input class="sys_menu_delete" type="hidden" name="cust_main_menu[delete][]" value="" />
														<input type="hidden" name="cust_main_menu[unid][]" value="<?= $menu['unid']; ?>" />
														<input type="hidden" name="cust_main_menu[parent][]" value="<?= $menu['parent']; ?>" />
														Label:&nbsp;<input id="trackback_url" class="code"  type="text" name="cust_main_menu[name][]" value="<?= $menu['name']; ?>" style="width:250px;" />
														<br/>
														Url:&nbsp;<input id="trackback_url" class="code"  type="text" name="cust_main_menu[url][]" value="<?= $menu['url']; ?>" style="width:250px;" />
														<br/>
														Class:&nbsp;<input id="trackback_url" class="code"  type="text" name="cust_main_menu[class][]" value="<?= $menu['class']; ?>" style="width:250px;" />
														<p class="submit">
								...
								<div id="major-publishing-actions">
									...
									<div id="publishing-action">
										<input class="button-primary" type="submit" value="Save" name="mm_save_options" />
									</div>
									<div class="clear"></div>
								</div>
							</form>
